    ...
    <div className="min-h-screen bg-page lg:grid lg:grid-cols-[248px_minmax(0,1fr)]">
      ...
      <aside id="teacher-navigation" className={cx("fixed inset-y-0 left-0 z-50 flex w-64 -translate-x-full flex-col bg-navy text-white shadow-xl transition-transform duration-200 lg:sticky lg:top-0 lg:h-screen lg:w-auto lg:translate-x-0 lg:shadow-none", open && "translate-x-0")}>
        ...
        <nav aria-label="담임 메뉴" className="flex-1 space-y-1 px-3 py-5">
          <NavLink href={`/teacher?class=${classCode}`} active={view === "dashboard"} onClick={() => setOpen(false)}>대시보드</NavLink>
          <NavLink href={`/teacher?class=${classCode}&view=students`} active={view === "students"} onClick={() => setOpen(false)}>학생 관리</NavLink>
          <NavLink href={`/teacher?class=${classCode}&view=links`} active={view === "links"} onClick={() => setOpen(false)}>링크 관리</NavLink>
          {(["출력/다운로드", "공지사항", "설정"] as const).map((label) => <div key={label} className="flex min-h-11 items-center justify-between rounded-lg px-3 text-sm text-slate-400" aria-disabled="true"><span>{label}</span><span className="text-[10px] font-semibold">준비 중</span></div>)}
        </nav>
