...
export function ConsultationStageNavigation({ current, onChange }: { current: ConsultationStage; onChange: (stage: ConsultationStage) => void }) {
  ...
  return <nav aria-label="상담 단계" className="sm:overflow-x-auto"><div className="grid grid-cols-2 gap-1 pb-1 sm:flex sm:min-w-max sm:border-b sm:border-line sm:pb-0">{stages.map((stage) => <button key={stage} type="button" aria-current={current === stage ? "step" : undefined} onClick={() => onChange(stage)} className={cx("min-h-11 min-w-0 w-full rounded-lg border px-2 py-2 text-center text-sm font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-inset sm:w-auto sm:rounded-none sm:border-0 sm:border-b-2 sm:px-4", current === stage ? "border-brand bg-blue-50 text-brand sm:bg-transparent" : "border-line bg-white text-muted hover:text-navy sm:border-transparent sm:bg-transparent")}>{CONSULTATION_STAGE_LABELS[stage]}</button>)}</div></nav>;
}
